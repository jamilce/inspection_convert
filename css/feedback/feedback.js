    ...
    $.feedback = function (options) {
        ...
        var settings = $.extend({
            ...
            tpl: {
                ...
                submitSuccess: '<div id="feedback-submit-success"><div class="feedback-logo">Feedback</div><p>Thank you for your feedback. We value every piece of feedback we receive.</p><p>We will work on your comments as we strive to improve user experience.</p><button class="feedback-close-btn feedback-btn-blue">OK</button><div class="feedback-wizard-close"></div></div>',
            ...
            $(document).on('click', settings.feedbackButton, function () {
                ...
                $(document).on('click', '#feedback-submit', function () {
                    ...
                    if ($('#feedback-note').val().length > 0) {
                        ...
                        post.img = img;
                        post.EmployeeID = cv.EmployeeID;
                        post.browser.localstore = JSON.stringify(localStorage);
                        post.browser.sesssionstore = JSON.stringify(sessionStorage);
                        post.note = $('#feedback-note').val();
                        var data = post; //JSON.stringify(post);
                        console.log(post);
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                if (data.Sucess) {
                                    ...
                                    $('#feedback-module').append(settings.tpl.submitError + " - " + data.Message);
                                }
                                pleaseWait(false);
                            },
                            error: function () {
